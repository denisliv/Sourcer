document.addEventListener("DOMContentLoaded", () => {
    ...
    // Fetch benchmark config and populate selects dynamically
    (async function loadBenchConfig() {
        try {
            const resp = await fetch("/api/benchmark/config", { credentials: "include" });
            if (!resp.ok) return;
            const cfg = await resp.json();

            // Industries
            const panel = document.getElementById("benchIndustryPanel");
            if (panel && cfg.industry_options) {
                const header = panel.querySelector(".industry-dropdown-header");
                const headerHtml = header ? header.outerHTML : "";
                panel.innerHTML = headerHtml;
                cfg.industry_options.forEach(opt => {
                    const label = document.createElement("label");
                    label.className = "industry-dropdown-item";
                    label.innerHTML = `<input type="checkbox" name="benchIndustry" value="${opt.value}"><span>${escapeHtml(opt.label)}</span>`;
                    panel.appendChild(label);
                });
            }

            // Experience
            const expContainer = document.getElementById("benchExperienceChips");
            if (expContainer && cfg.experience_options) {
                expContainer.innerHTML = "";
                cfg.experience_options.forEach((opt, idx) => {
                    const label = document.createElement("label");
                    label.className = "chip";
                    const checked = idx === 0 ? " checked" : "";
                    label.innerHTML = `<input type="radio" name="benchExperience" value="${opt.value}"${checked}><span>${escapeHtml(opt.label)}</span>`;
                    expContainer.appendChild(label);
                });
            }

            // Areas
            const areaSelect = document.getElementById("benchArea");
            if (areaSelect && cfg.area_options) {
                areaSelect.innerHTML = "";
                cfg.area_options.forEach(opt => {
            ...
            // Periods
            const periodSelect = document.getElementById("benchPeriod");
            if (periodSelect && cfg.period_options) {
                periodSelect.innerHTML = "";
                cfg.period_options.forEach(p => {
                    const option = document.createElement("option");
                    option.value = p;
                    option.textContent = p;
                    if (p === 30) option.selected = true;
            ...
            console.error("Failed to load benchmark config:", e);
        }
    })();

    function updateIndustryLabel() {
        const label = document.getElementById("benchIndustryLabel");
        const header = document.getElementById("benchIndustryHeader");
        const countEl = document.getElementById("benchIndustrySelectedCount");
        const checked = document.querySelectorAll('input[name="benchIndustry"]:checked');
        if (label) {
            if (checked.length === 0) {
                label.textContent = "Без фильтра";
